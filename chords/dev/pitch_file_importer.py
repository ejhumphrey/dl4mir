...
def create_note_entity(npz_file, lab_file, dtype=np.float32):
    ...
    note_numbers = [str(_) for _ in labels]
    entity.note_numbers = mir_eval.util.interpolate_intervals(
        intervals, note_numbers, entity.time_points, fill_value='[]')
...
def populate_stash(keys, cqt_directory, lab_directory, stash,
    ...
    for idx, key in enumerate(keys):
        ...
        stash.add(key, create_note_entity(cqt_file, lab_file, dtype))
