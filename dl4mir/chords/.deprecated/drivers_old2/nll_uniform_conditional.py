...
def main(args):
    ...
    root_idx = optimus.Input(
        name='root_idx',
        shape=(None,),
        dtype='int32')

    quality_idx = optimus.Input(
        name='quality_idx',
        shape=(None,),
        dtype='int32')
    ...
    root_classifier = optimus.Softmax(
        name='root_classifier',
        input_shape=layer3.output.shape,
        n_out=12,
        act_type='linear')

    quality_classifier = optimus.MultiSoftmax(
        name='quality_classifier',
        input_shape=layer3.output.shape,
        output_shape=(None, 12, 13),
        act_type='linear')

    all_nodes = [layer0, layer1, layer2, layer3,
                 root_classifier, quality_classifier]

    # 1.1 Create Losses
    root_nll = optimus.NegativeLogLikelihood(
        name="root_nll")

    quality_nll = optimus.ConditionalNegativeLogLikelihood(
        name="quality_nll")
    ...
    trainer_edges = optimus.ConnectionManager([
        ...
        (layer3.output, root_classifier.input),
        (layer3.output, quality_classifier.input),
        (root_classifier.output, root_nll.likelihood),
        (root_idx, root_nll.target_idx),
        (quality_classifier.output, quality_nll.likelihood),
        (quality_idx, quality_nll.conditional_idx),
        (root_idx, quality_nll.independent_idx)])
    ...
    update_manager = optimus.ConnectionManager([
        ...
        (learning_rate, root_classifier.weights),
        (learning_rate, root_classifier.bias),
        (learning_rate, quality_classifier.weights),
        (learning_rate, quality_classifier.bias)])

    print "Trainer"
    trainer = optimus.Graph(
        name=GRAPH_NAME,
        inputs=[input_data, root_idx, quality_idx, learning_rate],
        nodes=all_nodes,
        connections=trainer_edges.connections,
        outputs=[optimus.Graph.TOTAL_LOSS],
        losses=[root_nll, quality_nll],
        updates=update_manager.connections)

    optimus.random_init(root_classifier.weights)
    optimus.random_init(quality_classifier.weights)

    print "Validator"
    validator = optimus.Graph(
        name=GRAPH_NAME,
        inputs=[input_data, root_idx, quality_idx],
        nodes=all_nodes,
        connections=trainer_edges.connections,
        outputs=[optimus.Graph.TOTAL_LOSS],
        losses=[root_nll, quality_nll])

    root_posterior = optimus.Output(
        name='root_posterior')

    quality_posterior = optimus.Output(
        name='quality_posterior')
    ...
    predictor_edges = optimus.ConnectionManager([
        ...
        (layer3.output, root_classifier.input),
        (layer3.output, quality_classifier.input),
        (quality_classifier.output, quality_posterior),
        (root_classifier.output, root_posterior)])
    ...
    predictor = optimus.Graph(
        ...
        outputs=[root_posterior, quality_posterior])

    # 3. Create Data
    print "Opening data"
    stash = biggie.Stash(args.training_file)
    stream = S.minibatch(
        D.create_uniform_factored_stream(stash, TIME_DIM, vocab_dim=VOCAB),
        batch_size=50)
