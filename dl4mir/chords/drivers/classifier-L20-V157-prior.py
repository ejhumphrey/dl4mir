...
from ejhumphrey.dl4mir.chords import selectors as S
...
# Gross.
LOG_PRIOR = np.load("/home/ejhumphrey/Dropbox/NYU/dl4mir/log_prior.npy")
TRAIN_IDX = cPickle.load(open(
    "/media/attic/dl4mir/chord_estimation/chord_dsets/0/train_index.pk"))
...
def main(args):
    ...
    # 3. Create Data
    dset = optimus.File(args.training_file)
    chord_selector = S.ChordSelector(
        dset, S.entity_to_quality_index(VOCAB), TIME_DIM,
        prior=LOG_PRIOR, index=TRAIN_IDX)
    source = optimus.Queue(
        dset,
        transformers=[
            T.pitch_shift(8),
            T.map_to_index(VOCAB)],
        batch_selector=chord_selector,
        **SOURCE_ARGS)
