...
def main(args):
    ...
    trainer_edges = optimus.ConnectionManager([
        ...
        (quality_weight, chord_nll.weights)])
    ...
    trainer = optimus.Graph(
        name=GRAPH_NAME,
        inputs=[input_data, chord_idx, quality_weight, learning_rate],
        nodes=all_nodes,
    ...
    validator = optimus.Graph(
        name=GRAPH_NAME,
        inputs=[input_data, chord_idx, quality_weight],
        nodes=all_nodes,
        connections=trainer_edges.connections,
        outputs=[optimus.Graph.TOTAL_LOSS],
        losses=[chord_nll],
    ...
    source = optimus.Queue(
        ...
        transformers=[
            ...
            T.QualityObserver(min_weight=1.0/14)],
        **SOURCE_ARGS)
    print "Warming up weights..."
    for n in range(500):
        source.next()
    print "Done!"
